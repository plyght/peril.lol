...
export const BlogPostList: FC<Props> = ({ posts }) => {
  ...
  function filterPosts() {
    setFilteredPosts(
      posts.filter(({ data: { tags, title, description } }) => {
        const titleMatch =
          search.length === 0 ? true : title.toLowerCase().includes(search.toLowerCase());

        const descriptionMatch =
          search.length === 0 ? true : description.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="post-list">
      ...
      {filteredPosts.length === 0 ? (
        ...
        filteredPosts.map(({ data: { title, description }, id }) => (
